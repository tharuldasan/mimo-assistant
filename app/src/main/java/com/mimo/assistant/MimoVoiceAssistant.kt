...
class MimoVoiceAssistant(
    ...
    override fun onInit(status: Int) {
        ...
        textToSpeech?.let { MimoVoiceProfile.apply(it, SpokenLanguage.ENGLISH.locale) }
    }
    ...
    override fun onResults(results: Bundle?) {
        ...
        val shouldSpeak = transcript.contains("speak", ignoreCase = true) ||
            transcript.contains("read it", ignoreCase = true) ||
            transcript.contains("කියන්න")
        cloudMimoClient.ask(transcript, currentLanguage) { result ->
            mainHandler.post {
                val reply = result.getOrElse { error ->
                    "Mimo cloud error: ${error.message ?: "unknown error"}"
                }
    ...
    private fun speak(text: String) {
        ...
        textToSpeech?.let { MimoVoiceProfile.apply(it, language.locale) }
